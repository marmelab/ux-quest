...
function BookstoreAdmin() {
  ...
  const section: Section = (searchParams.get("section") as Section) || "books"
  const view: View = (searchParams.get("view") as View) || "list"
  const selectedId = searchParams.get("sid")
    ? Number(searchParams.get("sid"))
    : null
  ...
  const switchSection = (s: Section) => {
    navigate({ section: s, view: "list" })
  }
  const openShow = (id: number) => {
    navigate({ section, view: "show", sid: String(id) })
  }

  const openEdit = (id: number) => {
    navigate({ section, view: "edit", sid: String(id) })
    if (section === "books") {
  ...
  const saveBook = () => {
    if (view === "create") {
      setBooks((prev) => [
        ...prev,
        {
          ...
          authorId: parseInt(formData["authorId"] ?? "1") || 1,
    ...
    navigate({ section: "books", view: "list" })
  }
  ...
  const saveAuthor = () => {
    ...
    navigate({ section: "authors", view: "list" })
  }
  ...
  const renderContent = () => {
    if (section === "books") {
        ...
        return (
          <BookForm
            ...
            onCancel={() =>
              navigate(
                view === "create"
                  ? { section: "books", view: "list" }
                  : { section: "books", view: "show", sid: String(selectedId) }
              )
            }
      ...
      if (view === "list")
        return (
          <AuthorsList authors={authors} onShow={openShow} onEdit={openEdit} />
        )
        ...
        return (
          <AuthorForm
            ...
            onCancel={() =>
              navigate(
                view === "create"
                  ? { section: "authors", view: "list" }
                  : {
                      section: "authors",
                      view: "show",
                      sid: String(selectedId),
                    }
              )
            }
            isCreate={view === "create"}
              ...
              {view !== "list" && (
                <>
                  <Button
                    variant="link"
                    size="xs"
                    onClick={() => navigate({ section, view: "list" })}
